...
|---|---|---|---|---|
| 7.1 `contains` drawn as a labelled edge on the Module Schematic | **Synthesis, not a loss for either source.** Containment (section 4.1) is a relation stored as nesting — nothing changes there, and section 11.1's tier-3 edge-kind list stays closed to `covers`, `satisfies`, `documents`. The graph stores no `contains` edge at tier 3, so the loader and linter are unaffected. But whether the Module Schematic *draws* an arrow for that relation is a rendering question, and PRD 0.2 gives the wireframe that call: draw containment from the module root to each facet card as arrows carrying the wireframe's inline `contains` label. What's drawn is a rendering of nesting, not a stored edge — a user cannot create, delete, or reroute one, and it never appears in any edge count. | Section 11.1 answers "what edge kinds exist at tier 3," which the wireframe cannot override — no `contains` edge kind is ever created or persisted. Section 4.1 answers "is containment stored as an edge," and it isn't, here or anywhere. Neither section answers "does the Module Schematic draw a line for it," which is a visual/copy question squarely inside PRD 0.2's wireframe-wins rule, so the drawing wins there. | 5 (draws the arrows); 3 (the edge-rendering table gets a recorded exception: tier-3 `contains` renders as an unstored, non-interactive line) | Ruled |
...
| No zoom readout / no edge-kind legend chip on the Module Schematic (1d), unlike 1a and 1e | **Ruled: add both**, consistent with the other two tiers. PRD 12.1 already asserts the Module Schematic legend "reads `contains`, `covers`, `satisfies`" — per the corrected 10.1 row 1 ruling, `contains` genuinely is drawn at tier 3 (as an unstored rendering of nesting), so PRD 12.1's three-item legend was right as written; build the chip with all three labels. | 3, 5 |
| Header-dot fill colour varies per lifecycle-state example in 1b (grey / amber / green-dim / green-full / dim-grey) | **Ruled: keep it, as a redundant cue.** The dot carries a per-state fill because the wireframe draws one, but it is never the sole carrier of state — border weight, fill saturation, and overlay geometry carry the distinction that must still read after colour is removed, per PRD 12.7. | 4 |
...
One item remains Open — the unlabelled thumbnail in 1a — and it does not
block a wave: its row states the default to build to right now.
