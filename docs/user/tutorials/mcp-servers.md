# Give your agent HELVE's tools

*Agents · 7 min*

Let Claude Code or another MCP client drive the parts of HELVE you drive.

---

MCP — the Model Context Protocol — is how a coding agent picks up tools it
did not ship with. HELVE hosts its own servers, so an agent working in your
project can call into the parts of HELVE you would otherwise have to drive by
hand.
...
them. You bring your own — HELVE's job is to give it tools and a terminal it
can reach them from.
...
> every agent has those already. Only things that exist _inside HELVE and
...
_Turning Echo on writes `helve-echo` into the project's `.mcp.json`._

A `.mcp.json` at the project root, holding one entry per enabled server.
HELVE **merges** rather than overwrites. If the project already has servers
of its own, they are left exactly as they were — only the `helve-` prefixed
keys are HELVE's to write or remove.
...
Those two variables are set by **HELVE**, on the shells HELVE spawns. A
terminal you opened outside HELVE inherits neither, so the URL does not
...
> to. But it does mean the fix for "my agent cannot see the HELVE tools" is
> almost always **run the agent from a terminal inside HELVE** — see the
...
3. Approve the server when the client asks. A project-scoped `.mcp.json` is
   ...
   the client and HELVE does not try to defeat it silently.

Tool names arrive namespaced by the server they came from —
`mcp__helve-echo__ping` — because each server is its own endpoint rather than
...
> approval. The opt-in would pre-approve HELVE's own servers for a project.
...
> server that is off look the same from HELVE's side.

---

**Takeaway:** You can turn a HELVE MCP server on, get it into a project's
`.mcp.json`, and connect an agent to it from a HELVE terminal.
