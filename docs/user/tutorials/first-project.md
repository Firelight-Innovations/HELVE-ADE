...
*Getting started · 6 min · after [The OpenKaava window](the-window.md)*

Open a folder, set it up as an OpenKaava project, and know what got written.

---

A OpenKaava project is **a folder**. Not a workspace file, not a database entry —
a directory on disk that you point OpenKaava at.
...
2. Pick any folder. It does not have to be an OpenKaava project, or a git
   repository, or empty.
3. Home now shows it at the top, under **Open**, with its full path.

> The picker is a native OS dialog and it blocks until you answer it. If OpenKaava
...
A folder with no OpenKaava manifest still opens, and this is deliberate rather
than lenient. OpenKaava has to be able to point at a project that already exists,
...
OpenKaava project**. Nothing forces you to press it.

Open a plain folder → Home marks it **not set up** → Click **Set up as an OpenKaava project**
...
<!-- SCREENSHOT: a file explorer showing Anvil.kaava and .kaava/ added beside a folder's existing files, 480x300 -->

_`Anvil.kaava` and `.kaava/`, added beside the files the folder already had._

1. A manifest named after the folder — a project called `Anvil` gets
   `Anvil.kaava`. Small, hand-editable TOML, meant for version control.
2. A `.kaava/` directory beside it. This is the opposite of the manifest:
   everything OpenKaava _produces_ about the project — agent traces, designs,
...
what OpenKaava means when it says "this project": the path is not, since you will
...
two things OpenKaava wrote into it.
