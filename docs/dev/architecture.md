...
OpenKaava rather than work on it.
...
| Stack manifest | TOML | `kaava.toml` |
...
`kaava.toml` says which components make up the stack, and which version of each
...
Set `KAAVA_MANIFEST` to point a dev build at a different manifest.

## Layout

This repository is a Cargo workspace and a pnpm workspace at the same time. The
tool protocol needs libraries that *other* OpenKaava repositories depend on, so
...
kaava.toml            stack manifest: pinned component versions
...
crates/               Rust libraries shared with the tool repositories
  kaava-tool-manifest/  parses and validates kaava-tool.toml
  kaava-rpc/            JSON-RPC over the standard streams, both halves
packages/             npm packages shipped to the tool repositories
  bridge/               @openkaava/bridge: one tool frontend, either host
examples/
...
src/                  React frontend
  ...
  shell/                the frame tools run inside, one directory per region,
    ...
    WindowRoot.tsx        one OpenKaava window
...
src-tauri/            Rust backend
  src/
    ...
    manifest.rs         locating and parsing kaava.toml
    ...
    project/            what a project is, and which one is open
      ...
      marker.rs           the <name>.kaava manifest
      store.rs            the recents file, and what survives a restart
    mcp/                the MCP servers OpenKaava hosts for a coding agent
...
A project is **a folder**. The folder becomes a *OpenKaava* project when it holds a
`<name>.kaava` manifest. That manifest is small, hand-editable TOML, and is
meant for version control. A `.kaava/` directory sits beside it and holds
everything OpenKaava generates about the project: agent traces, designs, docs, and
...
A folder with no manifest still opens. That behaviour is intentional. OpenKaava can
open a project that already exists, and the answer to "what happens when the
`.kaava` format changes" must never be "the project stops opening". Home marks
...
command-line flag or a double-clicked `.kaava` file could do later, with no
...
`src/shell/WindowRoot.tsx` is one OpenKaava window. The frame is a title bar, a
...
it is tested against: a complete tool in miniature, with a `kaava-tool.toml`, a
core that speaks JSON-RPC over its standard streams, and a frontend whose only
host coupling is `@openkaava/bridge`. Copying it is the intended way to start a tool
repository.
...
surface is still blank. `crates/kaava-rpc` is the shell's half of the second
