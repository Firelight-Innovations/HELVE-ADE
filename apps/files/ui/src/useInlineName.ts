 ...
 * text still in it. That list is the thing worth having once: a copy of a
 * five-rule state machine is a copy that will disagree with the original the
 * first time either is touched, so this hook is the gesture and the two
 * components are its renderings.
 *
...
export function useInlineName({
   ...
   *
   * Enter commits and then, when the owner re-renders, the field is unmounted,
   * which fires `blur`. Without this the blur would commit the same name a
   * second time, or, on the Escape path, resurrect a name just abandoned.
   */
   ...
   * so typing a new name does not silently destroy the extension and, with it,
   * the file's language and its icon. That is VS Code's behaviour, and it is
   * the difference between rename being safe to use quickly and one you have to
   * be careful with. It happens once: after a failed attempt the user's own
   * edit is worth more than a helpful reselection of it.
   */
  useEffect(() => {
  ...
  const finish = (take: boolean) => {
    ...
    // required" about a field nobody filled in. A rename to the name it already
    // has is the same — the backend accepts it as a no-op, but going through
    // with it would re-list a folder and move the cursor for a change nobody
    // made, and opening rename and clicking away has to be free.
  ...
  return {
    ...
    onKeyDown: (event) => {
      ...
      // Enter — the tree's scrollport, the strip's roving focus. A text field
      // needs every one of those, so nothing from in here may reach them.
