 ...
 *
 * The shell's menu bar cannot reach into this app — Files runs in an iframe and
 * the shell must not import its hooks or poke its DOM. So a menu item is a
 * *message*: the shell posts a transport-B `command` to the active frame, and
 * this module is what the Files frame does about it. `docs/tool-protocol.md` §3
 * has the wire shape; `@helve/bridge`'s `onCommand` is the receiving end.
 *
 ...
 * crosses it, and the day Files becomes a tool repository of its own, nothing
 * in `apps/files/` may be reaching into `src/`. Two copies of a thirteen-line
 * table is the price of that boundary being real.
 */
...
import { describe, duplicate, saveAs, type Root } from "./rpc";
...
export interface MenuCommandDeps {
  ...
  /** Put the delete confirmation up. The same one the context menu raises. */
  askDelete: (target: DeleteTarget) => void;
  /** Something changed on disk that the tree cannot know about. Re-read it. */
  onTreeChanged: () => void;
...
export function useMenuCommands({
  ...
  const run = useCallback(
    ...
    [active, askDelete, explorer, files, onError, onTreeChanged],
  );
  ...
  useEffect(() => {
    ...
    if (root !== null) {
      available.push(COMMAND.newFile, COMMAND.trash);
    }
  ...
  }, [root, active, dirty, onDisk, editor]);
}
