...
export function toGraph(doc: SchematicDoc): SchematicGraph {
  ...
  return {
    ...
    // Every field mirrors `SchematicNode`, not only what the Outline and the
    // status bar happen to read — the Inspector (Wave 6) reads this same
    // projection (`App.tsx`'s `graph = toGraph(state.doc)`), and a field
    // dropped here is a field no Inspector tab can ever draw, regardless of
    // whether `buildDoc` carried it in.
    nodes: kept.map((node) => ({
      id: node.id,
      slug: node.slug,
      title: node.title,
